...
func (r *RouteRepo) GetByID(ctx context.Context, id uuid.UUID) (*planning.Route, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT r.id, r.type, COALESCE(rt.code, '') as type_code, COALESCE(rt.name, '') as type_name, r.date, r.daily_plan_id, r.notes, r.status, r.created_at
		 FROM planning.routes r
		 LEFT JOIN planning.route_types rt ON r.type = rt.id
		 WHERE r.id = $1`, id,
	).Scan(&rt.ID, &rt.Type, &rt.TypeCode, &rt.TypeName, &rt.Date, &rt.DailyPlanID, &rt.Notes, &rt.Status, &rt.CreatedAt)
...
func (r *RouteRepo) List(ctx context.Context, limit, offset int) (*repository.ListResult[planning.Route], error) {
	...
	for rows.Next() {
		var rt planning.Route
		if err := rows.Scan(&rt.ID, &rt.Type, &rt.TypeCode, &rt.TypeName, &rt.Date, &rt.DailyPlanID, &rt.Notes, &rt.Status, &rt.CreatedAt); err != nil {
