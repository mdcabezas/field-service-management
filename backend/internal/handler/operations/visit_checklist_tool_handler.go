...
type VisitChecklistToolHandler struct {
	repo repository.VisitChecklistToolRepository
}

func NewVisitChecklistToolHandler(repo repository.VisitChecklistToolRepository) *VisitChecklistToolHandler {
	return &VisitChecklistToolHandler{repo: repo}
}

func (h *VisitChecklistToolHandler) GetByID(c *gin.Context) {
...
func (h *VisitChecklistToolHandler) ListByVisit(c *gin.Context) {
...
func (h *VisitChecklistToolHandler) Create(c *gin.Context) {
	var body operations.VisitChecklistTool
...
func (h *VisitChecklistToolHandler) Update(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
...
func (h *VisitChecklistToolHandler) Delete(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
