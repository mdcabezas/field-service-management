...
type ChecklistTemplateToolHandler struct {
	repo repository.ChecklistTemplateToolRepository
}

func NewChecklistTemplateToolHandler(repo repository.ChecklistTemplateToolRepository) *ChecklistTemplateToolHandler {
	return &ChecklistTemplateToolHandler{repo: repo}
}

func (h *ChecklistTemplateToolHandler) GetByID(c *gin.Context) {
...
func (h *ChecklistTemplateToolHandler) ListByTemplate(c *gin.Context) {
	templateID, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
	}
	items, err := h.repo.ListByTemplate(c.Request.Context(), templateID)
	if err != nil {
		handler.HandleServiceError(c, err)
		return
	}
...
func (h *ChecklistTemplateToolHandler) Create(c *gin.Context) {
	var body operations.ChecklistTemplateTool
...
func (h *ChecklistTemplateToolHandler) Delete(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
