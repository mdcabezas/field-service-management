...
type VisitMeasurementHandler struct {
	repo repository.VisitMeasurementRepository
}

func NewVisitMeasurementHandler(repo repository.VisitMeasurementRepository) *VisitMeasurementHandler {
	return &VisitMeasurementHandler{repo: repo}
}

func (h *VisitMeasurementHandler) GetByID(c *gin.Context) {
...
func (h *VisitMeasurementHandler) ListByVisit(c *gin.Context) {
...
func (h *VisitMeasurementHandler) Create(c *gin.Context) {
	var body operations.VisitMeasurement
...
func (h *VisitMeasurementHandler) Update(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
...
func (h *VisitMeasurementHandler) Delete(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
