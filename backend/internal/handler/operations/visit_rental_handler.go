...
type VisitRentalHandler struct {
	repo repository.VisitRentalRepository
}

func NewVisitRentalHandler(repo repository.VisitRentalRepository) *VisitRentalHandler {
	return &VisitRentalHandler{repo: repo}
}

func (h *VisitRentalHandler) GetByID(c *gin.Context) {
...
func (h *VisitRentalHandler) ListByVisit(c *gin.Context) {
...
func (h *VisitRentalHandler) Create(c *gin.Context) {
	var body operations.VisitRental
...
func (h *VisitRentalHandler) Delete(c *gin.Context) {
	id, ok := handler.ParseUUID(c, "id")
	if !ok {
		handler.RespondError(c, http.StatusBadRequest, "invalid id")
		return
